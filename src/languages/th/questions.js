...
export default function QuestionsTh(){
  const { id } = useParams()
  ...
  const [hideAnswerBtn, setHideAnswerBtn] = useState('ซ่อนคำตอบ');
  ...
  const afterGameTexts = ['สนุกมั้ย?','ไว้มาเล่นกันอีกนะ','ไม่เป็นไร ครั้งหน้าเอาใหม่','อยากเล่นอีกมั้ย?']
  const [afterGameText, setAfterGameText] = useState(afterGameTexts[Math.floor(Math.random() * afterGameTexts.length)])

  var hook = sortedCategories.find(item => item.link == `/th/${id}`)
  ...
  const handleHideOrShowAnswer = () => {
    if (hideAnswerBtn === 'ซ่อนคำตอบ'){
      setHideAnswerBtn('แสดงคำตอบ');
    } else {
      setHideAnswerBtn('ซ่อนคำตอบ')
    }
    ...
    <div className={`App${isGoingBack ? ' fade-out-game' : ''}`}>
      <header className={`head-game${clearGame ? ' active' : ' inactive'}${startPage ? ' start' : ''}`}>
        <div className="sec-left">
          <h1 id="category">หมวดหมู่: {hook.category}</h1>
          <h1 id="get-point" className={`${correct ? 'animate' : ''}`}>คะแนน +1</h1>
        </div>
        <h1 id="timer">{count}</h1>
        <div className="checks-btns">
          <Button size='medium' onClick={handleHideOrShowAnswer}>{hideAnswerBtn}</Button>
          <Button size='medium' onClick={() => setGoBackModal(true)}>กลับไปหน้าแรก</Button>
        </div>
      </header>
      ...
      <Modal open={goBackModal} onClose={answerNo}>
        ...
        <div className="checks-btns with-top-space">
          <Button onClick={answerYes}>ใช่</Button>
          <Button onClick={answerNo}>ไม่</Button>
        </div>
      </Modal>

      {timesUp || isGoingBack && <InvisibleOverlay />}
      <BlockSpace isActive={timesUp} text={(<span style={{color:"#E63737"}}>หมดเวลา!</span>)}/>

      <div className="modal-center">
        <div className={`score-result${scoreResult ? ' active' : ''}`}>
          <h1 className='result-title'>ผลลัพธ์</h1>
          <h1 id='score'>คะแนน: {numPoint}</h1>
          <h2 className="after-game-text">{afterGameText}</h2>
          <div className="checks-btns with-top-space">
            <Button size='medium' onClick={backToHomepage}>กลับ</Button>
            <Button size='medium' onClick={playAgain}>เล่นอีกครั้ง</Button>
          </div>
        </div>
      </div>
    </div>
